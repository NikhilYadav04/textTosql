...
class RouterOutput(BaseModel):
    ...
    relevance: Literal["relevant", "irrelevant"] = Field(
        description="Whether the user query is relevant to a construction site management database."
    )
...
async def query_router_node(state: AgentState):
    ...
    Your task is to determine if the user's query is relevant to a construction site management database.
    The database contains information about projects, sites, contractors, workers, materials, suppliers, purchase orders, expenses, equipment, tasks, inspections, and daily logs.
