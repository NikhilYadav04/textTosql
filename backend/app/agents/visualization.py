...
from app.services.llm import get_openai_client, get_model_name
...
async def visualization_planner_node(state: AgentState):
    ...
    if tool_call:
        ...
        return {
            "needs_visualization": arguments.get("needs_visualization", False),
            "visualization_type": arguments.get("visualization_type")
        }
...
async def visualization_generator_node(state: AgentState):
    ...
    # Get chart type recommendation from planner
    viz_type = state.get("visualization_type", "bar")
    
    system_prompt = f"""You are a Vega-Lite expert.
    Generate a valid Vega-Lite JSON specification to visualize the provided data.
    
    User Query: {query_to_use}
    Recommended Chart Type: {viz_type}
    Data: {json.dumps(results)}
    
    Rules:
    1. Return ONLY the JSON object.
    2. Use the 'data' property with 'values' set to the provided data.
    3. Use a "{viz_type}" chart type as recommended, unless the data clearly suits another type better.
    4. Choose appropriate encodings based on the data types.
    5. Add a title and tooltips.
    6. Set "width": "container" to ensure it takes the full available width.
    7. Set "height": 300 for a good aspect ratio.
    8. Enable "autosize": {{{{ "type": "fit", "contains": "padding" }}}}.
    """
